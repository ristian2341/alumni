/**
	** DKDS JavaScript Widget Module
	** @author Dua Kelinci Developer
...
(function( $ ) {
	...
	*/
	...
	/** Table function */
	$.fn.dkdsTable = function(opt) {
		// render table fixed
		table.render(this);
		var setting = $.extend({}, table.config, opt), _this=this, arrObj = [];
		
		// jika enterKey: true
		if(setting.enterKey === true) enterKey.option("enable");
		// jika inputKey: true
		if(setting.inputKey === true) inputKey.option("enable");
		// if editabled on table: true
		if(setting.editableGrid === true) {
			$(document).on("click","[data-role='editable']", function(e) {
				e.preventDefault();
				_initAttr($(this));
			});
			
			$(document).on("click","[data-role='rollback']", function(e) {
				e.preventDefault();
				_initDefault($(this));
			});
			
			$(document).on("click","[data-role='commit']", function(e) {
				e.preventDefault();
				_initCallback();
				_initDefault($(this));
			});
			
			// jika proses e pake ajax, maka di init ulang
			$(document).ajaxComplete(function() {
				$(_this.selector).dkdsTable();
			});
		}
		// define max width, biar g mekar waktu click event edit
		_this.css("max-width", _this.css("width"));
		
		function _initAttr(el) {
			var $parent = el.parent(),
				arrInput = [], arrTmp = [];
			for(var i=1; i <= $parent.siblings("td:not(td:first)").length; i++) {
				var text_ = $parent.siblings("td").eq(i).text(),
					attr_ = el.parents("tbody").siblings("thead").find("th").eq(i),
					input_ = '<input type="text" class="form-control" id="'+attr_.attr("data-id")+'" name="'+attr_.attr("data-name")+'" value="'+text_+'">';
				arrInput.push(input_);
				var obj = {
					value: text_,
					button: $.trim($parent.html()),
				};
				arrObj.push(obj);
			}
			
			for(var j=0; j <= arrInput.length-1; j++) {
				$parent.siblings("td:not(td:first)").eq(j).html(arrInput[j]);
				var tmp = {
					value: $parent.siblings("td:not(td:first)").eq(j).find("input").val(),
				};
				arrTmp.push(tmp);
			}

			$("tr").attr("data-selected", 0);
			el.closest("tr").attr("data-selected", 1);
			
			if(el.closest("tr").attr("data-selected") == 1) {
				_this.find("a[data-role='editable']").not(el).attr("data-role", "not-editable");
			}
			
			if(typeof setting.options.readonly !== "undefined") {
				var split_ = setting.options.readonly.split(","),
					index;
				for(index in split_) {
					$("#"+$.trim(split_[index])).attr("readonly", 1);
				}
			}
			
			if(typeof setting.options.datepicker !== "undefined") {
				var split_ = setting.options.datepicker.split(","),
					index;
				for(index in split_) {
					$("#"+$.trim(split_[index])).datepicker({
						format: "dd-mm-yyyy",
					...
					// event init datepicker
					$(document).on("click","#"+split_[index], function(e) {
						e.preventDefault();
						var posY = $(this).offset().top-$(".datepicker").outerHeight();
						if(posY > is.trigger) {
							$(this).datepicker("show");
							$(".datepicker").css({ top: ($(this).position().top-$(this).outerHeight()-is.trigger) +'px' });
						} else {
							$(this).datepicker("show");
						}
					});
			...
			if(typeof setting.options.dropdown !== "undefined") {
				var object = ObjectToArray(setting.options.dropdown),
					split_id = object.property_id,
					split_values = object.property_values;
				for(var index in split_id) {
					var prop = split_id[index],
						name_ = $("#"+prop).attr("name"),
						value_ = $("#"+prop).val(),
						content = "";
					content += '<select name="'+name_+'" id="'+prop+'" value="'+value_+'" data-name="dropdown" data-search="true">';
					for(var x in split_values[index].split(",")) {
						content += '<option>'+$.trim(split_values[index].split(",")[x])+'</option>';
					}
					content += '</select>';
					$("#"+prop).replaceWith(content);
					
					// init dropdown select
					dropdown.init();
					$("#"+prop).val($("#"+prop).prev().attr("value"));
					// override width according to th width
					$("#"+prop).css({width:$("th[data-id='"+prop+"']").width() +'px'});
					
					var lontong = {
						posDropY: $("#"+prop).offset().top,
						dropMenuHeight: $("#"+prop).siblings(".drop-menu").outerHeight(),
						elemHeight: $("#"+prop).outerHeight()
					};
					is.arrGrid.push(lontong);
					is.grid = true;
				}
			}
			el.parent().html(_initBtn);
		}
		
		function _initBtn() {
			$container = $([
				'<a href="javascript:void(0)" class="btn btn-success btn-xs btn-flat" data-role="commit">',
					'<span class="glyphicon glyphicon-ok"></span>',
				'</a>',
				'<a href="javascript:void(0)" class="btn btn-danger btn-xs btn-flat ml1" data-role="rollback">',
		...
		function _initDefault(el) {
			var $parent = el.parent(),
				len = $parent.siblings("td:not(td:first)").length;
			for(var i = 0; i <= len-1; i++) {
				$parent.siblings("td:not(td:first)").eq(i).html("<td>"+arrObj[i].value+"</td>");
			}
			_this.find("a[data-role='not-editable']").attr("data-role", "editable");
			el.closest("tr").attr("data-selected", 0);
			el.parent().html(arrObj[0].button);
			arrObj = [];
		}
		...
		function _initCallback() {
			$.ajax({
				...
				beforeSend: function() {
					_this.loader("load");
					$(".loader-container").before("<div class=\"loader-wrapper\" />");
					$(".loader-wrapper").css({
						height: _this.height() - _this.find("thead").outerHeight() +'px',
						left: _this.position().left +'px',
						top: _this.position().top + _this.find("thead").outerHeight()+20 +'px',
						width: _this.width() +'px'
					});
					$(".loader-container").addClass("loader-ongrid");
				},
				success: function() {
					try {
						setting.source();
					} catch(e) {
						console.log("Errornya disini:", e.message);
					}
				},
				complete: function() {
...
var is = {
	arrGrid: [],
	grid: false, // kalau ada di dalam dkdsTable
...
/**
	...
	** contoh penggunaan ada di fungsi $.fn.dkdsTable
*/
...
var dropdown = {
	...
	event: function() {
		...
		$("body").off("click",".drop-toggle").on("click",".drop-toggle", function(e) {
			...
			// jika dropdown ada di dalam dkdsTable
			if(is.grid == true) {
				if($(this).next().hasClass("open")) {
					for(var i=0; i < is.arrGrid.length; i++) {
						var pos = is.arrGrid[i].posDropY-is.arrGrid[i].dropMenuHeight;
						if(pos > is.trigger) {
							$(this).next().css({ bottom: is.arrGrid[i].elemHeight-10 +'px', top: "unset" });
							$(this).next().fadeIn("fast");
						} else {
							$(this).next().css({ bottom: "unset", top: is.arrGrid[i].elemHeight +'px' });
							$(this).next().fadeIn("fast");
						}
					}
		...
		$("body").off("click",".drop-menu li").on("click",".drop-menu li", function(e) {
			...
			// kalau ada di dalam dkdsTable pake fadeOut
			if(is.grid == true) {
				$(this).closest(".drop-menu").fadeOut().removeClass("open");
			} else {
...
/**
	** function table grid
	** this function called on $.fn.dkdsTable();
*/
var table = {
	config: {
		editableGrid: false,
	...
	render: function(el) {
		var $thead = el.find("thead tr").first();
		$.each($thead, function(index, elem) {
			var len = $(elem).find("th[width]").length,
				$tbody = $(elem).parent().siblings("tbody");
			
			for(var i = 0; i <= len-1; i++) {
				$tbody.find("tr > td").eq(i).outerWidth($(elem).find("th[width]").eq(i).outerWidth());
			}
		});
		
		$.each($(".table-fixed"), function() {
			if($(this).parents(".table-fixed-container").length < 1) {
				$(this).wrap("<div class=\"table-fixed-container\">");
			}
		});
	}
};
/** End table grid function */
