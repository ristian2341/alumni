    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' =>!empty(Yii::$app->user->identity->developer)  || (!empty(Yii::$app->user->identity) && !empty(Yii::$app->user->identity->getMenu('Perusahaan')->create)),
                        'actions' => ['create'],
                        'roles' => ['@'],
                    ],
                    [
                        'allow' =>!empty(Yii::$app->user->identity->developer)  || (!empty(Yii::$app->user->identity) && !empty(Yii::$app->user->identity->getMenu('Perusahaan')->read)),
                        'actions' => ['index', 'view','send-register'],
                        'roles' => ['@'],
                    ],
                    [
                        'allow' =>!empty(Yii::$app->user->identity->developer)  || (!empty(Yii::$app->user->identity) && !empty(Yii::$app->user->identity->getMenu('Perusahaan')->update)),
                        'actions' => ['update'],
                        'roles' => ['@'],
                    ],
                    [
                        'allow' => !empty(Yii::$app->user->identity->developer)  || (!empty(Yii::$app->user->identity) && !empty(Yii::$app->user->identity->getMenu('Perusahaan')->delete)),
                        'actions' => ['delete'],
                        'roles' => ['@'],
                    ],
                ],
